...
export default function AdminGroupList() {
    ...
    */
    ...
    const handleGroupDelete = (groupId:string) => {
        axios.delete(`groups/${groupId}/delete`).then((res) => {
        }).catch((err)=>{})
    }
            ...
            <TableBody>
                {data.groups.map((group) => {
                    return <TableRow key={group.id}>
                        ...
                        <TableCell>
                        <Link href={`admin/groups/${group.id}/edit`}>編集</Link>
                        </TableCell>
                        <TableCell>
                            <Link href={`admin/groups/${group.id}/delete`}>削除</Link>
                        </TableCell>
                        <TableCell>
                            <Link href={`admin/groups/${group.id}/adduser`}>スーパーユーザー追加</Link>
                        </TableCell></TableRow>
                })}
            </TableBody>
        </Table>
